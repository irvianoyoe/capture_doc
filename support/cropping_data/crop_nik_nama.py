...
for image in images:
	...
	for (i, c) in enumerate(cnts):
		...
		if (w > 100 and w < 400) and (h > 11 and h < 22 and (y > 100 and y < 125)):
			...
			#print("masuk_if")
		if (w > 280 and w < 290) and (h > 15 and h < 40):
			locs.append((x, y, w, h))

	#potong
	print(locs)
	(x,y,w,h) = locs[0]
	img = image[y-5:y+h+5,x+3:x+w+3] # y = atas ke bawah y+h = bawah ke bawah x = kiri ke kanan x+w = kiri ke kiri
	(x,y,w,h) = locs[1]
	img2 = image[y-5:y+h+5,x-3:x+w+3]
	#img = imutils.resize(img, height=35, width=285)
	name1 = "nama%d" % (counter)
	name2 = "nik%d" % (counter)
	counter += 1
	cv2.imwrite("out/"+name1+".png", img)
	cv2.imwrite("out/"+name2+".png", img2)
	cv2.imshow("th3", img2)
	cv2.waitKey(0)