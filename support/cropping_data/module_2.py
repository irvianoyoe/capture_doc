...
# input : folder out hasil cropping
# output: folder out/out_transform hasil transformasi
#
# author: @kvkr
import cv2
import numpy as np
...
#u read folder
images = [cv2.imread(file,0) for file in glob.glob("out/ktp/*.png")]
counter = 0
...
for img in images:
	...
	name = "out"+`counter`
	counter += 1
	cv2.imwrite("out/ktp/transform/"+name+".png", img)

	#cv2.imwrite("out/out_transform/test.png", img)
	#cv2.imshow("th3", img)
	cv2.waitKey(0)